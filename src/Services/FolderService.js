...
async function getMainDirectories(reference_Id) {
    const response = await axiosInstance.get(`/${reference_Id}`);
    return response.data;
}
...
async function getAdirectory(reference_Id, directoryId) {
    const response = await axiosInstance.get(`/${reference_Id}/directories/${directoryId}`);
    return response.data;
}
...
async function moveDirectories(reference_Id, directoriesToMove, directoryToMoveTo) {
    const response = await axiosInstance.post(
        `/${reference_Id}/moveDirectories`,{ directoriesToMove,directoryToMoveTo}
    );
    return response.data;
}
...
async function getSubscriptionsByDirectoryId(reference_Id, directoryId) {
    const response = await axiosInstance.get(`/${reference_Id}/sharing/${directoryId}/sub`);
    return response.data;
}

// Function to rename a directory
async function renameDirectory(reference_Id, _id, name) {
    const response = await axiosInstance.post(`/${reference_Id}/renameDirectory`,{_id, name });
    return response.data
}
