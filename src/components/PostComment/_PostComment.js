...
function _PostComment({ post_id }) {
  ...
  useEffect(() => {
    if (comments.length > 0) {
      ...
      setAllComments(togetherComments);
    } else {
      setAllComments(addLocalComment);
    }
...
// (sqlalchemy.dialects.postgresql.asyncpg.Error) <class 'asyncpg.exceptions.ConnectionDoesNotExistError'>: connection was closed in the middle of operation [SQL: SELECT COUNT(*) FROM posts] (Background on this error at: https://sqlalche.me/e/20/dbapi)
